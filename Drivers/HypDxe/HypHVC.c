...
#define HVC_CALL_MASK            0xff00
#define HVC_LOG_CALL             0xff00
  ...
  )
{
  ...
  switch (HCall & HVC_CALL_MASK) {
  case HVC_LOG_CALL:
    Known = HypHVCLog(HCall, SystemContext);
    break;
  }

  if (!Known) {
    HLOG((HLOG_ERROR, "0x%lx: Ignoring Unknown HVC(%x) %lx %lx %lx %lx\n",
          SystemContext->ELR, HCall,
